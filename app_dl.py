...
def load_assets():
    ...
    # 3) Read training data – apply EXACT same pipeline as the training notebook:
    #    dropna(how="any") on ALL columns first, then language filter to "en",
    #    then keep only overall & reviewText
    df = pd.read_csv("amazon_reviews.csv")
    df = df.dropna(how="any")                        # matches notebook cell 6
    df = df[df["reviewText"].str.strip() != ""]

    # Language filter (notebook filtered to English only)
    try:
        ...
        def safe_detect(t):
            try:
                ...
                return "unknown"
        df["language"] = df["reviewText"].apply(safe_detect)
        df = df[df["language"] == "en"]
    except ImportError:
        pass  # langdetect not installed – skip language filter

    df = df.reset_index(drop=True)
    df = df[["overall", "reviewText"]].copy()

    # 4) Label mapping – same as notebook
    label_map = {1.0: 0, 2.0: 0, 3.0: 1, 4.0: 2, 5.0: 2}
    df["sentiment"] = df["overall"].map(label_map)
    df = df.dropna(subset=["sentiment"])

    # 5) Preprocessing – same as notebook (lowercase, remove punctuation, digits, newlines)
    df["reviewText"] = df["reviewText"].apply(preprocess_text)

    # 6) Refit TF-IDF with max_features derived from model input shape.
    #    The notebook did NOT pass stop_words to TfidfVectorizer, so we omit it
    #    here to reproduce the exact same vocabulary the model was trained on.
